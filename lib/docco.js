...
function *generateSections(filepath){
	...
	for(let lineIndex = 0; lineIndex < linesCount; lineIndex++){
		...
		do{
			if(lineModified){
				console.log(chalk.yellow('Line was modified, iterate again on ') + `"${chalk.bold.red(line)}"`);
			}
			lineModified = false;
			let matched;
			// Check opening multi-line comment
			matched = !lineModified && line.match( language.multi_start_matcher );
			if(!lineModified && matched){
				console.log('> Contains a mutli-line comment '+chalk.bold('start') + ':', matched);
				inMultiLineComment = true;
				// What is BEFORE the opening marker is code
				contentCode += line.slice(0, matched.index);
				// Reduce the line.
				line = line.slice(matched.index + matched[0].length);
				console.log({contentCode, contentDoc});
				lineModified = true;
				yield *doYieldSection();
			}
			// Check closing multi-line comment
			matched = !lineModified && inMultiLineComment && line.match( language.multi_end_matcher );
			if(!lineModified && matched){
				console.log('> Contains a mutli-line comment '+chalk.bold('end') + ':', matched);
				inMultiLineComment = false;
				// What is BEFORE the opening marker is code
				contentDoc += line.slice(0, matched.index);
				// Reduce the line.
				line = line.slice(matched.index + matched[0].length);
				console.log({contentCode, contentDoc});
				lineModified = true;
			...
			if(!lineModified){
				console.log('> Contains nothing special');
				if(inMultiLineComment){
					contentDoc += line;
					prevWasCode = false;
				} else {
					contentCode += (line + (prevWasCode ? '\n' : ''));
					prevWasCode = true;
				}
				console.log({contentCode, contentDoc});
			}
