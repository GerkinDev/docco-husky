...
( function() {
	...
	const generators = {
		...
		readme( context, package_json ) {
			...
			const templateFile = path.resolve(options.template_dir, 'readme.jade');
			const readme_template = jade.compile( fs.readFileSync( templateFile ).toString(), {
...
//							console.log(parsed);
	...
	const templateGen = context => {
		...
		const templateBuilder = filename => {
			...
			return args => {
				...
				const compiled = jade.compile( fs.readFileSync( templateFile ).toString(), {
					filename: templateFile,
				})(args);
	...
	const document = ( options, callback ) => {
		...
		context.templates = {
			docco: templateBuilder('docco.jade'),
			dox: templateBuilder('dox.jade'),
			content: templateBuilder('content.jade'),
			readme: templateBuilder('readme.jade'),
			styles: templateBuilder.styles,
		}

		return ensure_directory( options.output_dir ).then(() => {
			const promises = [
				promisify(fs.readFile)(path.resolve(options.template_dir, 'docco.css')).then(content => promisify(fs.writeFile)(path.resolve(options.output_dir, 'docco.css'), content.toString())),
